...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * temp = malloc(sizeof(*temp));
  temp->height = h;
  temp->width = w;
  temp->totalMines = numMines;
  if (temp == NULL) {
    perror("NEED MORE MEMORY");
    exit(EXIT_FAILURE);
  }
  ...
  while (i < h) {
    ...
    if (temp->board[i] == NULL) {
      perror("NEED MORE MEMORY");
      exit(EXIT_FAILURE);
    }
    for (int a = 0; a < w; a++) {
      temp->board[i][a] = UNKNOWN;
    }
    i++;
  }
  //for(int a = 0;a<h;a++){
  //for(int b = 0;b<w;b++){
  //  temp->board[a][b] = UNKNOWN;
  //}
  //}
  if (numMines < 0) {
    perror("wrong number for mines");
    exit(EXIT_FAILURE);
  }
